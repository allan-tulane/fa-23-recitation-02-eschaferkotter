...
**Name (Team Member 1):**Ethan Schaferkotter 
**Name (Team Member 2):**Izzy Blair
...
The asymptotic behavior of W(n) is O(n) if f(n) = 1. If f(n)= n, then the asymptotic behavior for W(n) is O(n*log(n)). If f(n) = log(n), then the bahavior is O((log(n))^2). When n = 10, a = 2 and b = 2, then the values of W(n) are 15 for f(n) = 1 and 36 for f(n)= n and ~16.2905 for f(n) = log(n).
